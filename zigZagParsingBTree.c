...
int height(struct btree *a)
{
if(a==NULL) return 0;
else
return (1+ max(height(a->rightchild),height(a->leftchild)));
}


void printLevel(struct btree *x, int l)
{
if(x==NULL)
   return;
...
printf("%d, ",x->num);
  ...
  printLevel(x->rightchild,l-1);
  printLevel(x->leftchild,l-1);
}
levelOrder(struct btree *a)
{
for(int h=1;h<height(a);h++)
printLevel(a,h);
}
void main()
{

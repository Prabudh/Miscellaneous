...
btree* mirrorTree(btree *node);
btree* mirrorTree(btree *node)
{
if(node == NULL) return NULL;
btree *newNode = (btree*)(malloc(sizeof(btree)));
newNode->num = node->num;
newNode->leftchild = mirrorTree(node->rightchild);
newNode->rightchild = mirrorTree(node->leftchild);
return newNode;
}
void main()
{
  ...
  zigzagParsing(mirrorTree(bt),stack1,stack2);  
return;
